...
module.exports = function (RED) {
    ...
    function AbraFlexiNabidkaVydanaNode(config) {
        ...
        node.on('input', function (msg, send, done) {
            ...
            fetchRecord(serverConfig, 'nabidka-vydana', id, function (err, json) {
                if (err) {
                    node.error(err.message, msg);
                    node.status({ fill: 'red', shape: 'ring', text: 'chyba' });
                    return done(err);
                }
                var records = json.winstrom && json.winstrom['nabidka-vydana'];
                if (!records || records.length === 0) {
                    node.warn('Záznam nenalezen: nabidka-vydana/' + id, msg);
                ...
                outMsg.topic            = 'nabidka-vydana';
                outMsg.abraflexi_id     = record.id || id;
                var subMsg              = RED.util.cloneMessage(msg);
                subMsg.payload         = record['nabidka-vydana-polozka'] || [];
                subMsg.topic           = 'nabidka-vydana-polozka';
    ...
    RED.nodes.registerType('abraflexi-nabidka-vydana', AbraFlexiNabidkaVydanaNode);
};
