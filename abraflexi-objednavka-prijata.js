...
module.exports = function (RED) {
    ...
    var READONLY_FIELDS = ["id", "lastUpdate", "updatedBy", "createdBy", "createdDate", "zamekK", "sumZklCelkem", "sumDphCelkem", "sumZklCelkemMen", "sumDphCelkemMen", "sumNaklady", "pocetPriloh", "faNazev2", "storno", "uzivatel", "sazbaDphOsv", "sazbaDphSniz", "sazbaDphSniz2", "sazbaDphZakl", "uuid", "procReal"];
    ...
    function AbraFlexiObjednavkaPrijataNode(config) {
        ...
        node.on('input', function (msg, send, done) {
            ...
            fetchRecord(serverConfig, 'objednavka-prijata', id, function (err, json) {
                if (err) {
                    node.error(err.message, msg);
                    node.status({ fill: 'red', shape: 'ring', text: 'chyba' });
                    return done(err);
                }
                var records = json.winstrom && json.winstrom['objednavka-prijata'];
                if (!records || records.length === 0) {
                    node.warn('Záznam nenalezen: objednavka-prijata/' + id, msg);
                ...
                outMsg.topic            = 'objednavka-prijata';
                outMsg.abraflexi_id     = record.id || id;
                var subMsg              = RED.util.cloneMessage(msg);
                subMsg.payload         = record['objednavka-prijata-polozka'] || [];
                subMsg.topic           = 'objednavka-prijata-polozka';
    ...
    RED.nodes.registerType('abraflexi-objednavka-prijata', AbraFlexiObjednavkaPrijataNode);
};
