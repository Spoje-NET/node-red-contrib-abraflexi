...
module.exports = function (RED) {
    ...
    var READONLY_FIELDS = ["id", "lastUpdate", "updatedBy", "createdBy", "createdDate", "zamekK", "sumZklCelkem", "sumDphCelkem", "sumZklCelkemMen", "sumDphCelkemMen", "sumNaklady", "pocetPriloh", "ucetni", "zuctovano", "storno", "uzivatel", "sazbaDphOsv", "sazbaDphSniz", "sazbaDphSniz2", "sazbaDphZakl", "uuid", "sparovano", "generovatSkl"];
    ...
    function AbraFlexiPokladniPohybNode(config) {
        ...
        node.on('input', function (msg, send, done) {
            ...
            fetchRecord(serverConfig, 'pokladni-pohyb', id, function (err, json) {
                if (err) {
                    node.error(err.message, msg);
                    node.status({ fill: 'red', shape: 'ring', text: 'chyba' });
                    return done(err);
                }
                var records = json.winstrom && json.winstrom['pokladni-pohyb'];
                if (!records || records.length === 0) {
                    node.warn('Záznam nenalezen: pokladni-pohyb/' + id, msg);
                ...
                outMsg.topic            = 'pokladni-pohyb';
                outMsg.abraflexi_id     = record.id || id;
                var subMsg              = RED.util.cloneMessage(msg);
                subMsg.payload         = record['pokladni-pohyb-polozka'] || [];
                subMsg.topic           = 'pokladni-pohyb-polozka';
    ...
    RED.nodes.registerType('abraflexi-pokladni-pohyb', AbraFlexiPokladniPohybNode);
};
