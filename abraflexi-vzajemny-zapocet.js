...
module.exports = function (RED) {
    ...
    function AbraFlexiVzajemnyZapocetNode(config) {
        ...
        node.on('input', function (msg, send, done) {
            ...
            fetchRecord(serverConfig, 'vzajemny-zapocet', id, function (err, json) {
                if (err) {
                    node.error(err.message, msg);
                    node.status({ fill: 'red', shape: 'ring', text: 'chyba' });
                    return done(err);
                }
                var records = json.winstrom && json.winstrom['vzajemny-zapocet'];
                if (!records || records.length === 0) {
                    node.warn('Záznam nenalezen: vzajemny-zapocet/' + id, msg);
    ...
    RED.nodes.registerType('abraflexi-vzajemny-zapocet', AbraFlexiVzajemnyZapocetNode);
};
