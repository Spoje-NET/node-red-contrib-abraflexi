...
module.exports = function (RED) {
    ...
    function AbraFlexiObjednavkaVydanaNode(config) {
        ...
        node.on('input', function (msg, send, done) {
            ...
            fetchRecord(serverConfig, 'objednavka-vydana', id, function (err, json) {
                if (err) {
                    node.error(err.message, msg);
                    node.status({ fill: 'red', shape: 'ring', text: 'chyba' });
                    return done(err);
                }
                var records = json.winstrom && json.winstrom['objednavka-vydana'];
                if (!records || records.length === 0) {
                    node.warn('Záznam nenalezen: objednavka-vydana/' + id, msg);
                ...
                outMsg.topic            = 'objednavka-vydana';
                outMsg.abraflexi_id     = record.id || id;
                var subMsg              = RED.util.cloneMessage(msg);
                subMsg.payload         = record['objednavka-vydana-polozka'] || [];
                subMsg.topic           = 'objednavka-vydana-polozka';
    ...
    RED.nodes.registerType('abraflexi-objednavka-vydana', AbraFlexiObjednavkaVydanaNode);
};
