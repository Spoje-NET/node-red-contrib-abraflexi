...
module.exports = function (RED) {
    ...
    function AbraFlexiFakturaVydanaNode(config) {
        ...
        node.on('input', function (msg, send, done) {
            ...
            fetchRecord(serverConfig, 'faktura-vydana', id, function (err, json) {
                if (err) {
                    node.error(err.message, msg);
                    node.status({ fill: 'red', shape: 'ring', text: 'chyba' });
                    return done(err);
                }
                var records = json.winstrom && json.winstrom['faktura-vydana'];
                if (!records || records.length === 0) {
                    node.warn('Záznam nenalezen: faktura-vydana/' + id, msg);
    ...
    RED.nodes.registerType('abraflexi-faktura-vydana', AbraFlexiFakturaVydanaNode);
};
