...
module.exports = function (RED) {
    ...
    function AbraFlexiPoptavkaVydanaNode(config) {
        ...
        node.on('input', function (msg, send, done) {
            ...
            fetchRecord(serverConfig, 'poptavka-vydana', id, function (err, json) {
                if (err) {
                    node.error(err.message, msg);
                    node.status({ fill: 'red', shape: 'ring', text: 'chyba' });
                    return done(err);
                }
                var records = json.winstrom && json.winstrom['poptavka-vydana'];
                if (!records || records.length === 0) {
                    node.warn('Záznam nenalezen: poptavka-vydana/' + id, msg);
                ...
                outMsg.topic            = 'poptavka-vydana';
                outMsg.abraflexi_id     = record.id || id;
                var subMsg              = RED.util.cloneMessage(msg);
                subMsg.payload         = record['poptavka-vydana-polozka'] || [];
                subMsg.topic           = 'poptavka-vydana-polozka';
    ...
    RED.nodes.registerType('abraflexi-poptavka-vydana', AbraFlexiPoptavkaVydanaNode);
};
